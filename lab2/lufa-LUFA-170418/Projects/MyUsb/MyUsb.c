  ...
  for (;;)
  {
    ...
    if (Endpoint_IsOUTReceived())
    {
      ...
      if (ReceivedData[4]==0) 
      {
        ...
        uint8_t  SendData[VENDOR_IO_EPSIZE];
        memset(SendData, 0x00, sizeof(SendData));
        uint8_t curLEDMask = LEDs_GetLEDs();
        SendData[0] = ((curLEDMask & LEDS_LED1) ? 1 : 0);
        SendData[1] = ((curLEDMask & LEDS_LED2) ? 1 : 0);
        SendData[2] = ((curLEDMask & LEDS_LED3) ? 1 : 0);
        SendData[3] = ((curLEDMask & LEDS_LED4) ? 1 : 0);
        SendData[4] = Temperature_GetTemperature();
